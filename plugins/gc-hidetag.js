//Código desarollado por Semi, https://wa.me/51922346462)
let handler = async (m, { conn, text, participants }) => {
  ...
  if (!m.quoted && type) {
    if ((mime === 'imageMessage')) {
      ...
      conn.sendMessage(m.chat, { text: text ? text : "@ApoloBot", mentions: participants.map(u => conn.decodeJid(u.id)) }, { quoted: m })
    }
  } else if (m.quoted) {
