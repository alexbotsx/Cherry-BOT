//Código desarollado por Semi, https://wa.me/51968914403)
let handler = async (m, { conn, text, participants }) => {
  ...
  if (!m.quoted && type) {
    if ((mime === 'imageMessage')) {
      ...
      conn.sendMessage(m.chat, { text: text ? text : "@CHERRYBOT", mentions: participants.map(u => conn.decodeJid(u.id)) }, { quoted: m })
    }
  } else if (m.quoted) {
